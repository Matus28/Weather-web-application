...
import { ErrorResponse } from 'src/middleware/requireAuth'
...
export const getDefaultCity = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user._id
...
export const addCity = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const city = (await isValidName) && !exist && City.create({ cityName, userId })
    res.status(200).json(city)
  } catch (error: unknown) {
    if (error instanceof Error) {
      res.status(400).json({ error: error.message })
...
export const setDefaultCity = async (req: Request, res: Response): Promise<void> => {
