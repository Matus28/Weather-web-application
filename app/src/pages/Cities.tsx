...
const Cities = (): JSX.Element => {
  ...
  const [selectedCity, setSelectedCity] = React.useState<WeatherData | null>(null)
  const { state: userValue } = useAuthContext()
  const { showSnackBar } = useSnackBar()
  const { data: cities } = useCities(userValue)
    ...
    <div className='main-container'>
      ...
      <div className='city-weather-container'>
        ...
        {weatherData.length > 0 && allLoaded && (
          <CityList
            cities={cities ?? []}
            weatherData={weatherData.map((element) => element.data as WeatherData)}
            selected={selectedCity?.location.name ?? ''}
            onSelect={handleSelect}
          />
        )}
        {weatherData.length > 0 && allLoaded && <SelectedCity data={selectedCity} />}
      </div>
    </div>
  )
}
