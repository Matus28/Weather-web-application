...
describe("selectImage function test", () => {
  test("returns URL for img of current day weather - Partly cloudy", () => {
    expect(
      selectImage(testWeatherCondition, true, "current", testConditionImagesURL)
    ).toBe("url/current/day-partly-cloudy");
  });
  ...
  test("returns URL for img of current night weather - Partly cloudy", () => {
    expect(
      selectImage(
        ...
        false,
        "current",
        testConditionImagesURL
      )
    ).toBe("url/current/night-partly-cloudy");
  });
  ...
  test("returns URL for img of forecast day weather - Partly cloudy", () => {
    ...
    ).toBe("//cdn.weatherapi.com/weather/64x64/day/116.png");
  });
  ...
  test("returns URL for img of forecast night weather - Partly cloudy", () => {
    ...
    ).toBe("//cdn.weatherapi.com/weather/64x64/night/116.png");
  });
});
