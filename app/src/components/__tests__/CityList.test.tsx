...
describe('CityList component test', () => {
  test('returns elements representing 2 cities (Bratislava, Kosice)', () => {
    const { container } = render(
      <CityList cities={testCities} weatherData={testWeatherData} onSelect={handleSelect} />,
    )
  ...
  test('returns elements where Bratislava is selected', () => {
    ...
    const divSelectedCity = container.getElementsByClassName('card-body active')
    const divCityName = divSelectedCity[0].getElementsByClassName('city-block-description__name')[0]
