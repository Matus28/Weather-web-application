...
describe("SelectedCity page test", () => {
  ...
  test("renders all main information about weather data of selected city (Bratislava)", () => {
    const { container } = render(<SelectedCity data={testBAWeatherData} />);
    const temperature = container.getElementsByClassName(
      "selected-city-weather__temp"
    )[0];
    const city = container.getElementsByClassName(
      "selected-city-weather__city"
    )[0];
    const rain = container.getElementsByClassName(
      "selected-city-weather__rain"
    )[0];
  ...
  test("renders today's forecast information", () => {
    const { container } = render(<SelectedCity data={testBAWeatherData} />);
    ...
    const divForecastList = container.getElementsByClassName(
      "today-forecast-list"
    )[0].childNodes;
