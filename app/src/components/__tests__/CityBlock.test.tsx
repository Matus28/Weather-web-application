...
describe('CityBlock component test', () => {
  ...
  test('renders div element for city with name Bratislava with all information', () => {
    ...
    const divCityName = container.querySelector('.city-block-description__name')
    const divCityTime = container.querySelector('.city-block-description__time')
    const divCityTemp = container.querySelector('.city-block-temp')
    const imgWeather = screen.getByAltText('Symbol image of weather') as HTMLImageElement
  ...
  test('renders button element to delete city from the list', () => {
    ...
    const buttonRemove = container.getElementsByTagName('button')[0] as HTMLButtonElement
